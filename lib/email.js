...
function parseTemplate(templateFile) {
  ...
  const yamlStarts = lines.findIndex(line => line.indexOf('---') === 0);
  const yamlEnds = lines.indexOf('---', yamlStarts+1);

  const headerText = lines.slice(yamlStarts+1,yamlEnds).join('\n');
  return {
    header: yaml.safeLoad(headerText),
    text: lines.slice(yamlEnds+1).join('\n')
  };  
}
...
exports.htmlMessage = function htmlMessage(templateFile) {
  const template = parseTemplate(templateFile);
  return (data) => {
    const html = markdown.toHTML(Mustache.render(template.text, data));
    const plainText = Mustache.render(template.text, data);
    const attachments = template.header.attachments.map(attachment =>
...
if (!imaps.ImapSimple.prototype.append) {
  imaps.ImapSimple.prototype.append = function (message, options, callback) {
    var self = this;
